...
* DB2:          https://www.ibm.com/docs/en/db2/11.5?topic=statements-create-table
* Oracle:       https://docs.oracle.com/en/database/oracle/oracle-database/21/sqlrf/CREATE-TABLE.html
* MS-SQL:       https://learn.microsoft.com/en-us/sql/t-sql/statements/create-table-transact-sql?view=sql-server-ver16
...
* All DBMS listed above support signed integer data type with the following variance:
    *   |DBMS       |i8        |u8        |i16       |u16      |i32      |u32          |i64          |u64          |i128         |u128         |
        ...
        | SQLite    |`INT`     |`INT`     |`INT`     |`INT`    |`INT`    |`INT`        |`INT`        |`INT`        |`INT`        |`INT`        |
...
* All DBMS listed above support fixed and variable character type with the following variance:
    *   |DBMS       |Type           |    Max _s_|Default|Synonym|
        |-----------|---------------|----------:|------:|-------|
        | DB2       |`CHAR(` _s_ `)`|        254|       |       |
        ...
        | MySQL     |`CHAR(` _s_ `)`|        255|       |       |
        | PostgreSQL|`CHAR(` _s_ `)`| 10,485,760|       |`BPCHAR[(` _s_ `)]`|
        * where the elements between the brackets are optional.
...
* DBMS specific identity column options.
    *   |DBMS       |Column Definition|
        ...
        * MySQL increment size need to be set at the database level with:
            * `SET GLOBAL auto_increment_increment=` _incr_`;`
...
*   |Primitives Type                |Size| C |C# |GO |Java|Python|Rust| V |Zig| |DB2           |Oracle         |MS-SQL        |SQLite|MySQL         |PostgreSQL |
    |-------------------------------|:--:|:-:|:-:|:-:|:--:|:----:|:--:|:-:|:-:|-|--------------|---------------|--------------|------|--------------|------------|
    |bool \ boolean                 | 1  | Y | Y | Y | Y  | Y    | Y  | Y | Y | |`BOOLEAN`     |`NUMBER(1)`    |`BIT`         |`INT` |`BOOLEAN`     |`BOOLEAN`   |
    |byte                           | 1  |   | Y | Y | Y  |      |    |   |   | |`GRAPHIC(1)`  |`RAW(1)`       |`VARBINARY(1)`|`BLOB`|`BINARY(1)`   |`BIT(1)`    |
    |sbyte                          | 1  |   | Y |   |    |      |    |   |   | |`SMALLINT`    |`SMALLINT`     |`TINYINT`     |`INT` |`TINYINT`     |`SMALLINT`  |
    |char <sup>1</sup>              | 1  | Y |   |   |    |      |    |   |   | |`CHAR(1)`     |`CHAR(1)`      |`CHAR(1)`     |`TEXT`|`CHAR(1)`     |`CHAR(1)`   |
    |char <sup>2</sup>              | 2  |   | Y |   | Y  |      | Y  |   |   | |`CHAR(2)`     |`CHAR(2)`      |`CHAR(2)`     |`TEXT`|`CHAR(2)`     |`CHAR(2)`   |
    |i8                             | 1  |   |   | Y |    |      | Y  | Y | Y | |`SMALLINT`    |`SMALLINT`     |`TINYINT`     |`INT` |`TINYINT`     |`SMALLINT`  |
    |u8 <sup>a</sup>                | 1  |   |   | Y |    |      | Y  | Y | Y | |`SMALLINT`    |`SMALLINT`     |`SMALLINT`    |`INT` |`SMALLINT`    |`SMALLINT`  |
    |i16                            | 2  |   |   | Y |    |      | Y  | Y | Y | |`SMALLINT`    |`SMALLINT`     |`SMALLINT`    |`INT` |`SMALLINT`    |`SMALLINT`  |
    |u16 <sup>b</sup>               | 2  |   |   | Y |    |      | Y  | Y | Y | |`INTEGER`     |`INTEGER`      |`INTEGER`     |`INT` |`INTEGER`     |`INTEGER`   |
    |i32                            | 4  |   |   | Y |    |      | Y  | Y | Y | |`INTEGER`     |`INTEGER`      |`INTEGER`     |`INT` |`INTEGER`     |`INTEGER`   |
    |u32 <sup>c</sup>               | 4  |   |   | Y |    |      | Y  | Y | Y | |`DECIMAL(10)` |`NUMBER(10)`   |`BIGINT`      |`INT` |`BIGINT`      |`BIGINT`    |
    |i64                            | 8  |   |   | Y |    |      | Y  | Y | Y | |`DECIMAL(20)` |`NUMBER(20)`   |`BIGINT`      |`INT` |`BIGINT`      |`BIGINT`    |
    |u64 <sup>d</sup>               | 8  |   |   | Y |    |      | Y  | Y | Y | |`DECIMAL(20)` |`NUMBER(20)`   |`NUMERIC(20)` |`INT` |`DECIMAL(20)` |`NUMERIC(20)`|
    |i128                           | 8  |   |   |   |    |      | Y  |_Y_| Y | |`DECIMAL(31)` |`NUMBER(31)`   |`NUMERIC(31)` |`INT` |`DECIMAL(31)` |`NUMERIC(31)`|
    |u128 <sup>e</sup>              | 8  |   |   |   |    |      | Y  |_Y_| Y | |`DECIMAL(31)` |`NUMBER(31)`   |`NUMERIC(31)` |`INT` |`DECIMAL(31)` |`NUMERIC(31)`|
    |short                          | 2  | Y | Y |   | Y  |      |    |   |   | |`SMALLINT`    |`SMALLINT`     |`SMALLINT`    |`INT` |`SMALLINT`    |`SMALLINT`  |
    |ushort <sup>b</sup>            | 2  |   | Y |   |    |      |    |   |   | |`INTEGER`     |`INTEGER`      |`INTEGER`     |`INT` |`INTEGER`     |`INTEGER`   |
    |unsigned short <sup>b</sup>    | 2  |   |   |   |    |      |    |   |   | |`INTEGER`     |`INTEGER`      |`INTEGER`     |`INT` |`INTEGER`     |`INTEGER`   |
    |int                            | 4  | Y | Y | Y | Y  | Y    |    | Y |   | |`INTEGER`     |`INTEGER`      |`INTEGER`     |`INT` |`INTEGER`     |`INTEGER`   |
    |uint <sup>c</sup>              | 4  |   | Y | Y |    |      |    |   |   | |`DECIMAL(10)` |`NUMBER(10)`   |`NUMERIC(10)` |`INT` |`DECIMAL(10)` |`BIGINT`    |
    |unsigned int <sup>c</sup>      | 4  | Y |   |   |    |      |    |   |   | |`DECIMAL(10)` |`NUMBER(10)`   |`NUMERIC(10)` |`INT` |`DECIMAL(10)` |`BIGINT`    |
    |long                           | 8  | Y | Y |   | Y  |      |    |   |   | |`DECIMAL(20)` |`NUMBER(20)`   |`NUMERIC(20)` |`INT` |`DECIMAL(20)` |`NUMBER(20)`|
    |ulong <sup>e</sup>             | 8  |   | Y |   |    |      |    |   |   | |`DECIMAL(20)` |`NUMBER(20)`   |`NUMERIC(20)` |`INT` |`DECIMAL(20)` |`NUMBER(20)`|
    |unsigned long <sup>e</sup>     | 8  | Y |   |   |    |      |    |   |   | |`DECIMAL(20)` |`NUMBER(20)`   |`NUMERIC(20)` |`INT` |`DECIMAL(20)` |`NUMBER(20)`|
    |long long                      |16  | Y |   |   |    |      |    |   |   | |`DECIMAL(31)` |`NUMBER(31)`   |`NUMERIC(31)` |`INT` |`DECIMAL(31)` |`NUMBER(31)`|
    |unsigned long long <sup>e</sup>|16  | Y |   |   |    |      |    |   |   | |`DECIMAL(31)` |`NUMBER(31)`   |`NUMERIC(31)` |`NUM` |`DECIMAL(31)` |`NUMBER(31)`|
